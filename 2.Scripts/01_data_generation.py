...
DB_CONFIG = {
    ...
    'database': 'fleetlogix',
    'user': 'postgres',
    'password': 'hDiosfe7',  # Cambiar por tu contraseña
    'port': 5432
}
...
class DataGenerator:
    ...
    def generate_vehicles(self, count=200):
        ...
        for i in range(count):
            ...
            license_plate = f"{fake.random_uppercase_letter()}{fake.random_uppercase_letter()}{fake.random_uppercase_letter()}{random.randint(100,999)}{random.randint(0,9)}"
    ...
    def generate_routes(self, count=50):
        """Generar exactamente 50 rutas entre las 5 ciudades principales"""
        logging.info(f"Generando {count} rutas...")
        
        routes = []
        route_counter = 1
        
        # 1. Generar rutas base (tu lógica original)
            ...
            for destination in self.cities:
                if origin != destination:
                    ...
                    for i in range(num_routes):
                        ...
                        routes.append((route_code, origin, destination, round(distance, 2), round(duration, 2), toll_cost))
                        route_counter += 1
        
        # 2. Bucle de seguridad: Si faltan rutas para llegar a 50, generarlas extra
        while len(routes) < count:
            origin, destination = random.sample(self.cities, 2)
            route_code = f"R{str(len(routes) + 1).zfill(3)}"
            base_distance = self._get_distance(origin, destination)
            distance = base_distance + random.uniform(-50, 50)
            avg_speed = random.uniform(60, 80)
            duration = distance / avg_speed
        ...
        logging.info(f" {len(routes)} rutas insertadas")
    ...
    def generate_deliveries(self, total_needed=400000):
        """Generar exactamente 400000 entregas"""
        logging.info(f"Generando exactamente {total_needed} entregas...")
        ...
        for trip_id, departure, arrival, total_weight, city in trips_data:
            ...
            for i in range(num_deliveries):
                # FRENO DE EMERGENCIA: Si ya llegamos, salimos inmediatamente
                if delivery_counter >= total_needed:
                    break
                ...
                deliveries.append((
                    ...
                    float(round(package_weight, 2)), scheduled, delivered,
                    delivery_status, signature
                ))
        ...
        """
        execute_batch(self.cursor, query, deliveries, page_size=1000)
        self.connection.commit()
        
        self.counters['deliveries'] = len(deliveries)
        logging.info(f"Generación exacta de {len(deliveries)} entregas completada.")
    ...
    def generate_maintenance(self, count=5000):
        """Generar exactamente 5000 registros de mantenimiento"""
        logging.info(f"Generando exactamente {count} registros de mantenimiento...")
            ...
            if first_trip and last_trip:
                ...
                for i in range(num_maintenance):
                    ...
                    maintenance_records.append((
                        vehicle_id, maintenance_date, maint_type, description,
                        round(cost, 2), next_maintenance, performed_by
                    ))
        
        # Inserción batch: limitamos explícitamente a 'count' por seguridad
        query = """
        ...
        # Tomamos solo hasta 'count' registros para asegurar la precisión
        final_batch = maintenance_records[:count]
        execute_batch(self.cursor, query, final_batch, page_size=100)
        self.connection.commit()
        
        self.counters['maintenance'] = len(final_batch)
        logging.info(f"Generación exacta de {self.counters['maintenance']} mantenimientos completada.")
